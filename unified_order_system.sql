...
-- 2. 修改 payment_order 表，添加商品订单相关字段
-- 注意：如果字段已存在会报错，可以忽略或手动检查
ALTER TABLE `payment_order` 
ADD COLUMN `store_id` BIGINT COMMENT '取货门店ID（商品订单）';

ALTER TABLE `payment_order` 
ADD COLUMN `pick_up_time` TIMESTAMP NULL COMMENT '取货时间';

ALTER TABLE `payment_order` 
ADD COLUMN `finish_time` TIMESTAMP NULL COMMENT '完成时间';
