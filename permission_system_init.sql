...
-- ========================================
-- 重要说明：
-- 1. 如果 sys_role 表的字段已存在，下面的 ALTER TABLE 语句会报错，这是正常的，可以忽略
-- 2. 建议使用 permission_system_init_safe.sql（使用存储过程，不会报错）
-- 3. 或者手动检查表结构，只执行缺少的字段
-- ========================================

-- 1. 为 sys_role 表添加缺失字段
-- 如果字段已存在，会报错 "Duplicate column name"，可以忽略并继续执行后续语句
